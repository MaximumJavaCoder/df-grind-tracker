...
async function handleGrinderManufacturerInput(input){input.dataset.manufacturerId=''; let model=input.closest('[data-grinder-row]').querySelector('.g-model'); if(model){model.dataset.modelId='';model.dataset.manufacturerId=''} let q=input.value.trim(); if(q.length<2){renderAutocomplete(input,[]);return} let data=await grinderFetch(`/api/grinders/manufacturers/autocomplete?q=${encodeURIComponent(q)}&limit=4`); renderAutocomplete(input,data?.results||[],item=>{input.value=item.manufacturerName||item.label; input.dataset.manufacturerId=item.manufacturerId||item.id; if(model)model.dataset.manufacturerId=input.dataset.manufacturerId; renderAutocomplete(input,[]); h(8);});}
async function handleGrinderModelInput(input){input.dataset.modelId=''; let q=input.value.trim(); if(q.length<1){renderAutocomplete(input,[]);return} let row=input.closest('[data-grinder-row]'), manufacturerId=row.querySelector('.g-name')?.dataset.manufacturerId||input.dataset.manufacturerId||''; let qs=`q=${encodeURIComponent(q)}&limit=4${manufacturerId?`&manufacturerId=${encodeURIComponent(manufacturerId)}`:''}`; let data=await grinderFetch(`/api/grinders/models/autocomplete?${qs}`); renderAutocomplete(input,data?.results||[],item=>{input.value=item.modelName||item.label; input.dataset.modelId=item.modelId||item.id; input.dataset.manufacturerId=item.manufacturerId||manufacturerId; let m=row.querySelector('.g-name'); if(m&&!m.dataset.manufacturerId&&item.manufacturerName){m.value=item.manufacturerName;m.dataset.manufacturerId=item.manufacturerId||''} renderAutocomplete(input,[]); h(8);});}
